...
```{r cache=TRUE}
coda::traceplot(MVMREx$MR_Coda[, "theta"])
coda::densplot(MVMREx$MR_Coda[, "theta"])
```
...
We can supply a single value which will be used for all $\beta_{Xk}$-$\beta_Y$ and $\beta_{Xk}$-$\beta_{Xk}$ correlations, or we can provide the correlation matrix. It is assumed that the omegas are known, they can be determined by the proportion of sample overlap and estimated, e.g., using LD Score regression.
...
The matrix should contain omega values for $\beta_{Xk}$-$\beta_Y$ correlations in the first row and column, and then $\beta_{Xk}$-$\beta_{Xk}$ correlations in the rest. Note that each $\omega$ may take a different value.
...
```{r}
omega_mat = matrix(1, 3 ,3)
omega_mat[upper.tri(omega_mat)] = c(0.2, 0.2, 0.5)
omega_mat[lower.tri(omega_mat)] = c(0.2, 0.2, 0.5)
omega_mat
```
